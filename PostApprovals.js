...
const PostApprovals = () => {
  ...
  useEffect(() => {
    const getPendingPosts = async () => {
      try {
        setIsLoading(true);
          let pageData = await fetchPendingPosts(
  activeTab,
...
// If activeTab is visitLogs, call the second fetch function (fetchPendingPosts2)
if (activeTab === "visitLogs") {
  let pageData2 = await fetchPendingPosts(
    "visitLogs2", // pass a custom flag for the second collection
    filterData.searchValue,
    filterData.sortBy,
    cursorFields.lastVisible,
    cursorFields.pageSize,
  ...
  // Merge old and new logs
  pageData.records = [...pageData.records, ...pageData2.records];
  pageData.totalRecords += pageData2.totalRecords;
}

setCursorFields(prev => ({
  ...prev,
  lastVisible: pageData.lastDoc,
  pageHistory: pageData.pageHistory
}));

setTotalPages(pageData.totalRecords);

if (cursorFields.direction === "next") {
  setCurrentPageLength(prev => prev + pageData.records.length);
}
...
// Sort By Function
  ...
  useEffect(() => {
    ...
    // Fetch counts from old collections
    const { outreachCount, visitLogCount: oldVisitLogCount } = await fetchTotalCountOfPendingPosts();

    // Fetch counts from the new collection
    const { visitLogCount: newVisitLogCount } = await fetchTotalCountOfPendingPosts("visitLogs2"); // You need to pass the collection name

    // Update the state with the combined count
    setPageRecordCount({
      outreachCount: outreachCount,
      visitLogCount: oldVisitLogCount + newVisitLogCount
})
    }
